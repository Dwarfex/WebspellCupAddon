...
$type = mysqli_real_escape_string($_GET['type']);
   ...
   switch($type) {
	   ...
	   break;
	   case 'tickID':  $title = 'Ticket ID #'.$channelID;
   }
   ...
   elseif($type=='tickID') {
	  ...
	  if(valid_ticketer($channelID,$userID) OR iscupadmin($userID)) {
                 $validation_pass = "true";
	  }
	  else{
                 $validation_pass = "Insufficient permissions";
...
function delMessage(){
	$.ajax({ 
		type: "GET",
		url: "some.php",
		data: "action=del&type=<?php echo $type; ?>&id=<?php echo $channelID; ?>",   
...
if($loggedin) {
   ...
   if(iscupadmin($userID) || $userID==$channelID){
	          $admin = '<center><a onclick="delMessage();" style="cursor:pointer;">'.$_language->module['del_message'].'</a></center>';
   }
