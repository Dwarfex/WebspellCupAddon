...
//-- GLOBALS --//
  ...
  if($set['ccupgamelimit']==0 && !$loggedin) {
    ...
    if(isset($_GET['memberID'])) {
        $clanID = $_GET['memberID'];
    } 
	elseif(isset($_GET['clanID'])) {
        $clanID = $_GET['clanID'];
    }	

    $team = (isset($_GET['type']) && $_GET['type']==1 ? 0 : 1);
    ...
    elseif((isset($_GET['type']) && $_GET['type']=="ladder") || isset($_GET['laddID'])) { 
...
//-- MATCH REPORTING --//
        ...
        if(isset($cupID) && isset($clanID)) {
        
          if(isset($_GET['laddID']) && $display != 'team')
            ...
            $clanname = (empty($cc[$id]) ? "(DELETED)" : getname1($clanID,$cupID,0,strtolower($cuptitle)));            
	        ...
	        if($cuptitle=="Cup" && $display != 'team') {
	        
	           include ("title_cup.php");
	           $participants = (isset($_GET['type']) && $_GET['type']==1 ? "Players" : "Teams");	   	   
	        ...
	        if($display=='team') { 
			   ...
			   $scr_head  = ((isset($_GET['cupID']) && is1on1($cupID)) || (isset($_GET['laddID']) && ladderis1on1($_GET['laddID'])) ? 'Player' : 'Team');
			   ...
			   if($matchstatus2['report']) {
			          ...
			          $date_width = 30;
                      $report_head = '<td bgcolor="'.$bg1.'" align="center"><b>Status</b></td>';
			   }
		       ...
		       if(mysql_num_rows($getmatches)) {
	                 ...
	                 while($dm = mysql_fetch_array($getmatches)) 
	                  { 
	                     ...
	                     $match = '<img src="images/cup/icons/go.png"> <a href='.matchlink($matchid,0,$tg=0,$redirect=0).'><b>'.$matchid.'</b></a>';
	                     ...
	                     if($clanID == $dm['clan1']){
	                                 $score1 = $dm['score1'];
	                                 $score2 = $dm['score2'];		                                                                
	                                 $clanname = (empty($cc1[$id]) ? "(DELETED)" : getname1($dm['clan1'],$cupID,0,strtolower($cuptitle)));
    	                             $opponent = (empty($cc2[$id]) ? "(DELETED)" : getname1($dm['clan2'],$cupID,0,strtolower($cuptitle)));	                                 
	                                 ...
	                                 $score1 = $dm['score2'];
	                                 $score2 = $dm['score1'];	
	                                 $clanname = (empty($cc2[$id]) ? "(DELETED)" : getname1($dm['clan2'],$cupID,0,strtolower($cuptitle)));
    	                             $opponent = (empty($cc1[$id]) ? "(DELETED)" : getname1($dm['clan1'],$cupID,0,strtolower($cuptitle)));	                                 
...
//-- View matches by user or team --//
            ...
            }elseif(isset($clanID) && $clanID!=2147483647){  
		    ...
		    eval ("\$clans_details_title = \"".gettemplate("clans_details_title")."\";");
	            echo $clans_details_title;
                 ...
                 $one = (isset($_GET['memberID']) ? 1 : 0); 
                 $one_query = ($one ? "1on1='1'" : "1on1='0'");
                 $one_type = (isset($_GET['memberID']) ? "memberID" : "clanID");
                 ...
                 elseif(isset($_GET['type']) && $_GET['type']=="ladder") {
                 ...
                 $getcups = safe_query("SELECT * FROM ".PREFIX."cup_matches WHERE $one_query && type='$query_where' && (clan1='$clanID' || clan2='$clanID') GROUP BY $query_grouping ORDER BY date DESC");
                   if(!mysql_num_rows($getcups)) echo "<br /><br /> <strong>No $no_match_return matches</strong>";
                     ...
                     $cupname = ($cuptitle=="cup" ? getcupname($cupID) : getladname($cupID));
					 $cuptype = ($cuptitle=="cup" ? 'cupID' : 'laddID');
                     $leaguetype = getleagueType($da['matchID']);                 
                     $clanname = getname1($clanID,$cupID,0,$cuptitle); 
                     $clantag = getname1($clanID,$cupID,0,$cuptitle);
					 
					 if(isset($_GET['type']) && $_GET['type']=='gs') {					 
					       $cupn = '<a href="?site=groups&'.$cuptype.'='.$cupID.'"><img src="images/cup/icons/add_result.gif" align="right" border="0" alt="All matches for this cup"></a> <a href="'.$lk_details.$cupID.$t_gs_ex.'">'.$cupname.'</a>'; 					 
                     }
					 else{
					       $cupn = '<a href="?site=matches&action=viewmatches&'.$cuptype.'='.$cupID.$t_gs_ex.'"><img src="images/cup/icons/add_result.gif" align="right" border="0" alt="All matches for this cup"></a> <a href="'.$lk_details.$cupID.$t_gs_ex.'">'.$cupname.'</a>';
                     }
    	                      ...
    	                      while($dm = mysql_fetch_array($getmatches)) 
    	                       { 
                                  ...
                                  $date = date('l M dS Y', $dm['date']);								  
	                                 ...
	                                 $details.= '&nbsp;<a href='.matchlink($matchid,0,$tg=0,$redirect=0).'><img border="0" src="images/icons/foldericons/folder.gif"></a>';
									 $memberID = isset($_GET['memberID']) ? $_GE['memberID'] : 0;
	                              
	                                     if($clanID == $dm['clan1'])
	                                     {
	                                             $score1 = $dm['score1'];
	                                             $score2 = $dm['score2'];		                                 
	                                             $clanname = getname1($dm['clan1'],$cupID,0,strtolower($cuptitle));
	                                             $opponent = getname1($dm['clan2'],$cupID,0,strtolower($cuptitle));
								            
											if(isset($_GET['type']) && $_GET['type']=='gs') {
	                                             $teammatches = '<a href="?site=matches&action=viewmatches&'.$one_type.'='.$dm['clan1'].'&type=gs"><img border="0" src="images/cup/icons/add_result.gif" align="right" width="16" height="16"></a>';
	                                             $oppmatches =  '<a href="?site=matches&action=viewmatches&'.$one_type.'='.$dm['clan2'].'&type=gs"><img border="0" src="images/cup/icons/add_result.gif" align="left" width="16" height="16"></a>';
									        }
									        else{
	                                             $teammatches = '<a href="?site=matches&action=viewmatches&clanID='.$dm['clan1'].'&'.$cuptype.'='.$cupID.($memberID ? "&type=1" : "").'"><img border="0" src="images/cup/icons/add_result.gif" align="right" width="16" height="16"></a>';
	                                             $oppmatches =  '<a href="?site=matches&action=viewmatches&clanID='.$dm['clan2'].'&'.$cuptype.'='.$cupID.($memberID ? "&type=1" : "").'"><img border="0" src="images/cup/icons/add_result.gif" align="left" width="16" height="16"></a>';
									        }
	                                     }
	                                     else
	                                         {
         	                                     $score1 = $dm['score2'];
	                                             $score2 = $dm['score1'];	
	                                             $clanname = getname1($dm['clan2'],$cupID,0,strtolower($cuptitle));
	                                             $opponent = getname1($dm['clan1'],$cupID,0,strtolower($cuptitle));
				...
				if(mysql_num_rows($getmatches)) {
    	                      ...
    	                      while($dm = mysql_fetch_array($getmatches)) 
    	                       { 
	                              ...
	                              $game = '<img src="images/games/'.$image.'.gif" width="20" height="20" border="0">';
	                              $date = date('l M dS Y', $dm['date']);
	                              $vs = '<a href='.matchlink($matchid,0,$tg=0,$redirect=0).'><strong>vs.</strong></a>'; 
	                              ...
	                              $details = '<a href='.matchlink($matchid,0,$tg=0,$redirect=0).'><img src="images/icons/foldericons/folder.gif"></a>';
        ...
        $row = $result->fetch_assoc();
					...
					$cuptype = $typee;
	                        ...
	                        $game = '<img src="images/games/'.$image.'.gif" width="20" height="20" border="0">';
	                              ...
	                              $clanname = (empty($cc1[$id]) ? "(DELETED)" : getname1($row['clan1'],$cupID,0,strtolower($cuptitle)));
    	                          $opponent = (empty($cc2[$id]) ? "(DELETED)" : getname1($row['clan2'],$cupID,0,strtolower($cuptitle)));
                                  ...
                                  if($gt_lg_tp=='matchno') {
	                                     $teammatches = '<a href="?site=matches&action=viewmatches&'.$tag.'='.$row['clan1'].'&type=gs"><img border="0" src="images/cup/icons/add_result.gif" align="left" width="16" height="16"></a>';
	                                     $oppmatches =  '<a href="?site=matches&action=viewmatches&'.$tag.'='.$row['clan2'].'&type=gs"><img border="0" src="images/cup/icons/add_result.gif" align="right" width="16" height="16"></a>';
								  }
								  else{
