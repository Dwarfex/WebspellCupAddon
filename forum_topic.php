	...
	elseif($addreply && !$dt['closed']) {
		if($loggedin and $writer) {
          ...
          <tr bgcolor="'.PAGEBG.'"><td colspan="2"></td></tr>';
			...
			$mysqli_notify = mysqli_num_rows(safe_query("SELECT notifyID FROM ".PREFIX."forum_notify WHERE userID='".$userID."' AND topicID='".$topic."'"));
			$notify = ($mysqli_notify || $post_notify == '1') ? 'checked="checked"' : '';
