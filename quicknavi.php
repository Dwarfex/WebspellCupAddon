	  ...
	  $b_rows = mysqli_num_rows($team_cups);
            while($ds=mysqli_fetch_array($team_cups))
             {    
	             ...
	             $clanleave = '<option value="0" selected="selected"> - Select Team - </option>';	                 
	             $clanleave.= '<option name="clanID" value="'.cupParticipantID($userID,$ds['ID']).'">'.getclanname(cupParticipantID($userID,$ds['ID'])).'</option>';
            
				 if(iscupparticipant($userID,$ds['ID'],2)) {
	                   $clancheck = '<option value="0" selected="selected"> - Select Team - </option>';	                 
	                   $clancheck.= '<option name="clanID" value="'.cupParticipantID($userID,$ds['ID']).'">'.getclanname(cupParticipantID($userID,$ds['ID'])).'</option>';
				 }
				 else
				 {
				       $clancheck = $clan;
                 ...
                 $autocheckin = '<form action="index.php">
                                   ...
                                   <input type="hidden" name="cupID" value="'.$ds['ID'].'">
                                   <select name="clanID">'.$clancheck.'</select>
                 ...
                 $leavecup = '<form action="index.php">
                                ...
                                <input type="hidden" name="cupID" value="'.$ds['ID'].'">
                                <select name="clanID">'.$clanleave.'</select>
	     ...
	     $a_rows = mysqli_num_rows($one_cups);
           while($ds=mysqli_fetch_array($one_cups))
            {   
               ...
               $register = (isladparticipant($userID,$cupID,0) ? '(Already Entered)' : '<img border="0" src="images/cup/new_message.gif"> <a href="?site=ladders&action=register&clanID='.$userID.'&laddID='.$cupID.'" onclick="return confirm(\'Ladder starts on '.$start.' GMT. \');"><b>Signup</b></a>');
	  ...
	  $b_rows = mysqli_num_rows($team_cups);
           while($ds=mysqli_fetch_array($team_cups))
            {    
	             ...
	             $clanleave = '<option value="0" selected="selected"> - Select Team - </option>';	                 
	             $clanleave.= '<option name="clanID" value="'.ParticipantID($userID,$ds['ID']).'">'.getclanname(ParticipantID($userID,$ds['ID'])).'</option>';

				 if(isladparticipant($userID,$ds['ID'],2)) {
	                   $clancheck = '<option value="0" selected="selected"> - Select Team - </option>';	                 
	                   $clancheck.= '<option name="clanID" value="'.ParticipantID($userID,$ds['ID']).'">'.getclanname(ParticipantID($userID,$ds['ID'])).'</option>';
				 }
				 else
				 {
				       $clancheck = $clan;
                 ...
                 $is_groups = ($ds['gs_start'] <= time() && $ds['gs_end'] >= time() ? 1 : 0);
                 $game = '<img src="images/games/'.$ds['game'].'.gif" border="0" align="left" width="20" height="20">';
                 $register = (isladparticipant($userID,$cupID,0) ? '(Already Entered)' : $detail);
		 ...
		 $str_qn = strlen(getladname($cupID))-$length_qn;
		  ...
		  if($type!='ladders') {
		     $checkin_tb = '<td bgcolor="'.$bg1.'" align="center">'.$checkin_info.'</td>';
		  }
