...
if(!$cpr || !ca_copyr()) die();
  ...
  if(isset($_GET['cupID'])) {
      ...
      $thescore2 = "score2";
      $t_name = "cupID";
      $t_name2=(isset($_GET['matchID']) ? "ladderis1on1" : "is1on1");
      $t_name3 = "getcupname";
 }else{
      getladtimezone();
...
if(isset($_GET['cupID']) && isset($_GET['match']) && !isset($_GET['type'])) {
   ...
   $team2 = '<a href="?site=clans&action=show&clanID='.$dd['clan2'].'"><b>'.getclanname($dd['clan2']).'</b></a>';
		...
		if($dd['screens'] && $dd['screen_upper'] && $dd['screen_name']) {
			...
			foreach($screens as $screen) {
				if(!empty($screen)) {
					$screenshots.='<tr>
							  <td align="left" width="100%" bgcolor="'.$bg1.'"><a href="images/cup-screens/'.$screen.'" target="_blank"><b>'.(empty($screen_name[$n]) ? 'Screenshot' : $screen_name[$n]).'</b></a> - '.$_language->module['uploaded_by'].' <a href="?site=profile&id='.$screen_upper[$n].'">'.getnickname($screen_upper[$n]).'</a> '.(iscupadmin($screen_upper[$n]) ? "(admin)" : "").'</td>
						       </tr> ';
					$n++;
				}
				unset($screen);
		...
		if(is1on1($cupID)){
			...
			if(isset($_GET['type']) && $_GET['type']=='gs') {
			  ...
			  $tic=mysqli_fetch_array($ticket);
			  
			  if(mysqli_num_rows($ticket)) $pt = '<img src="images/cup/icons/ticket.png"> <a href="?site=cupactions&action=mytickets&tickID='.$tic['ticketID'].'"><strong>Protest Ticket</strong></a>'; else $pt = '';
			...
			if($dd['report_team2'])
			  ...
			  if(mysqli_num_rows($ticket)) $pt = '<img src="images/cup/icons/ticket.png"> <a href="?site=cupactions&action=mytickets&tickID='.$tic['ticketID'].'">Protest Ticket</a>'; else $pt = '';
		...
		$requests = '';
		if(mysqli_num_rows($requests_sql)){
			$requests='<br />
						<table bgcolor="'.$border.'" cellpadding="'.$cellpadding.'" width="100%">
							<tr>
			...
			while($du=mysqli_fetch_array($requests_sql)){
				if(!is1on1($cupID)){
					if(isleader($userID,$dd['clan1']))
...
$getrequests = safe_query("SELECT * FROM ".PREFIX."cup_requests WHERE matchID='".$dd['matchID']."'");
  if(mysqli_num_rows($getrequests)) {
  ...
  $mm_requests = '<table width="100%" bgcolor="'.$border.'" cellspacing="'.$cellspacing.'" cellpadding="'.$cellpadding.'">
   ...
   $no = 1;
    while($rq=mysqli_fetch_array($getrequests)) {
...
}elseif(isset($_GET[$t_name]) && (isset($_GET['match']) || $_GET['matchID'])) {

$laddID = $_GET[$t_name];
$cupID = $_GET[$t_name];
$ID = isset($_GET['match']) ? $_GET['match'] : 0;
...
if($t_name2($laddID)) {
  ...
  elseif(getleagueType($dd['matchID'])=="ladID") {
     ...
     $cc = mysqli_fetch_array($check_c);
	 
     if($cc['chalID']==$dd['matchno'] && mysqli_num_rows($check_c)) {
            $stage = 'Match Play';
 ...
 if(isset($map_option1)) {
	...
	$matchno = "-undefined-";
       ...
       else
       {
		...
		$ml = 'index.php'.matchlink($dd['matchID'],0,0,1);
		$parentID = $dd['matchID'];
		$comments_allowed = $dd['comment'];
		$type = "cm";
		$referer = "$ml";
