...
/* MESSAGE SENT SOUND NOTIFICATION */

if($donnees['type']=='userID') {

if($donnees['sound']==1 && $donnees['channelID']==$userID AND $donnees['pseudo']!=$donnees['channelID']) { 
   echo '<embed height="0" width="0" src="images/cup/message.mp3">';
   safe_query("UPDATE ".PREFIX."tchat SET sound='0' WHERE ID='".$donnees['ID']."'");
}

if($donnees['sound']==1 && $donnees['channelID']==$donnees['pseudo'] AND $userID!=$donnees['pseudo']) { 
   echo '<embed height="0" width="0" src="images/cup/message.mp3">';
   safe_query("UPDATE ".PREFIX."tchat SET sound='0' WHERE ID='".$donnees['ID']."'");
}
...
if($donnees['sound']==1 && $userID!=$donnees['pseudo']) { 
   echo '<embed height="0" width="0" src="images/cup/message.mp3">';
   safe_query("UPDATE ".PREFIX."tchat SET sound='0' WHERE ID='".$donnees['ID']."'");
}
...
elseif(isset($_POST['action']) AND isset($_POST['pseudo']) AND isset($_POST['message']) AND $_POST['pseudo'] != "" AND $_POST['message'] != "" AND $_POST['action'] == "envoi" ){
	...
	$pseudo = $_POST['pseudo'];
	    ...
	    $site_url = "?site=shout&id={$post_id}&type={$post_type}";
...
elseif(isset($_GET['action']) AND $_GET['action'] == 'del'){
	safe_query("DELETE FROM ".PREFIX."tchat WHERE channelID='".$_GET['id']."' && type='".$_GET['type']."'");
	echo'';
}//del message 1by1
...
echo '<div id="live">';
    ...
    if(isset($_GET['call']) && $_GET['call']=='now') {
       redirect('some.php?action=call&id='.$channelID.'&call=fail', '', 20);
    }
	
	echo '<table width="100%" border="0" cellspacing="4" cellpadding="4" bgcolor="#DDDDDD">';
    
    if(is_array($db)) {
       echo '<tr><td align="center" bgcolor="#F6F6F6"><strong>The user is now in the chat.</strong></center><script>setTimeout("window.close()",3000)</script></td></tr>';
    }    
    elseif($_GET['call']=='fail'){
       safe_query("UPDATE ".PREFIX."whoisonline SET `call`='0', calltimer='0' WHERE userID='$channelID'");
       echo '<tr><td align="center" bgcolor="#F6F6F6"><strong>No answer or unable to connect to '.getnickname($channelID).'</strong><br/> User maybe inactive or have pop-up blocker.<script>setTimeout("window.close()",3000)</script></td></tr>';
    }
    elseif($_GET['call']=='hangup'){
       echo '<tr><td align="center" bgcolor="#F6F6F6"><strong>You terminated the call.</strong><script>setTimeout("window.close()",3000)</script></td></tr>';
    }
    elseif($ds['call']==1) {
       echo '<tr><td align="center" bgcolor="#F6F6F6"><strong>Waiting for '.getnickname($channelID).'</strong><img src="images/cup/period_ani.gif"><br /><a href="some.php?action=call&id='.$channelID.'&call=hangup">Terminate?</a><br /></td></tr>';
    }
    else{
       echo '<tr><td align="center" bgcolor="#F6F6F6"><strong>Establishing connection</strong><img src="images/cup/period_ani.gif"></center></td></tr>';
