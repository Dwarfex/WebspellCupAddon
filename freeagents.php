...
$sys_err = '<div class="errorbox"><strong>Fatal error: Please seek assistance</strong></div>';
...
if(isset($_GET['action']) && $_GET['action']=='new') {
  ...
  if($loggedin && !is_array($cin)) {
   ...
   $cups = '<option value="0" selected">-- Select League --</option><optgroup label="Tournaments">';
    ...
    $query = safe_query("SELECT * FROM ".PREFIX."cup_ladders WHERE 1on1='0' && agents='1' ORDER BY ID DESC");
      while($cids2=mysql_fetch_array($query))
      {
        ...
        elseif(!is_array($db) && $cids2['status']==2 && !isladparticipant_memin($userID,$cids2['ID'],$checkin=0) && $cids2['sign']==1) 
		{
            $cups .= '<option value="?site=freeagents&action=new&ladderID='.$cids2['ID'].'" STYLE="color: #FFFFFF; font-weight: bold; font-size: 12px; background-color: '.$wincolor.';">'.getladname($cids2['ID']).'</option>';
            $fa[] = true;
		}
   ...
   if(!$fa) { echo '<div '.$error_box.'> There are no leagues in Sign-up phase.</div>'; }
   ...
   echo '<tr>
	 ...
	 <form action="" name="post" method="post">
	      <tr>
	       <td bgcolor="'.$pagebg.'" colspan="6"><img src="images/cup/icons/contact_info.png"> Available tournaments must be on sign-up phase.</td>
	     </tr>
	      <tr>
    ...
    if($table=="cup_ladders" && $cup_info['status']==2) {
	      $show_avail = '<strong>Note:</strong> The ladder has already started and you can participate immediately after acceptance of a team.'; 
	}
	else{
	      $show_avail = '<strong>'.date('l M dS Y \@\ g:i a', $cup_info['start']).'</strong><br /><select name="avail"><option value="0" selected="selected">-- Are you available on the above date/time? --</option>'.$dd_avail.'</select>';
	}
		 ...
		 <tr>
	      ...
	      <tr>
	       <td bgcolor="'.$bg1.'" colspan="6"><div class="errorbox"><img src="images/cup/icons/warning.png"> You will be automatically removed if you are already a participant of the league.</div></td>
	     </tr>
		  <tr>
		   <td bgcolor="'.$bg1.'"></td>
   ...
   echo '<tr>
	...
	if($clanID && $invite) {
	  ...
	  $query = safe_query("SELECT userID FROM ".PREFIX."cup_clan_members WHERE userID='$invite' && agent='1'");	  
        ...
        elseif($lg_type($userID,$cupID)!=$clanID) {	
			    echo '<div class="errorbox"><a href="javascript:history.go(-1);"><img src="images/cup/icons/goback.png" width="16" height="16" border="0"></a> <font color="red"><strong>Team selected is not registered on this league.</strong></font></div>';
		}
        elseif(isleader($userID,$clanID)) {	 		
                ...
                redirect('?site=clans&action=show&clanID='.$clanID.'#members', '<div '.$error_box.'>Successfully added as agent!</div>', 2);				
	    }
	    else{	   
	   ...
	   $cup_info=mysql_fetch_array(safe_query("SELECT * FROM ".PREFIX."$table WHERE ID='$cupID'"));
        ...
        case 0: $avail = 'Agent registered on Started phase'; break;			
	    }
		   ...
		   if($loggedin && $userID==$agent && $userID!=0 && $userID!='') {
		      ...
		      if($_GET['depart']=='true') {
			    ...
			    redirect('?site=freeagents', '<div '.$error_box.'>Successfully departed as agent <img src="images/cup/success.png"></div>', 2);
		      }
		   }
		   ...
		   if(!mysql_num_rows($agents_lp)) $no_team = '<a href="?site=clans&action=clanadd">(leading no team - create team now!)</a>';		
	           ...
	           while($dm=mysql_fetch_array($agents_lp)) {                     
		        ...
		        if($ds['ladID'] && $ds['cupID']==0) {
				       $sh_lg = 'laddID';
					   $sh_id = $ds['ladID'];
				}
				elseif($ds['cupID'] && $ds['laddID']==0){
				       $sh_lg = 'cupID';
					   $sh_id = $ds['cupID'];
				}
				else{
                   ...
                   $invite_to_team = '<form action="?site=freeagents" method="post">
										...
										<input type="hidden" name="'.$sh_lg.'" value="'.$sh_id.'">
                                      <select name="clanID">
				                      '.$clan.'
				                      </select>
            ...
            if($st['status']==1 OR ($ds['ladID'] && $ds['cupID']==0 && $st['status']==2 && $st['sign']==1)) {
		       ...
		       if($ds['cupID'] && $ds['ladID']==0 && $recruitees_tourna==1 && $userID!=$agent && !iscupparticipant($agent,$ds['cupID'])) {
		          $recruit = $invite_to_team;
		       }
		       elseif($ds['ladID'] && $ds['cupID']==0 && $recruitees_ladder==1 && $userID!=$agent && !isladparticipant($agent,$ds['ladID'])) {
		          $recruit = $invite_to_team;
		       }
		       elseif($ds['cupID'] && $ds['ladID']==0 && $recruitees_tourna==0 && iscupparticipant($userID,$ds['cupID']) && $userID!=$agent && !iscupparticipant($agent,$ds['cupID'])) {
		          $recruit = $invite_to_team;
		       }
		       elseif($ds['ladID'] && $ds['cupID']==0 && $recruitees_ladder==0 && isladparticipant($userID,$ds['ladID']) &&  $userID!=$agent && !isladparticipant($agent,$ds['ladID'])) {
		          $recruit = $invite_to_team;
		       }
		...
		if($ds['info']) {
		    $show_info = '<tr>
                            <td bgcolor="'.$bg1.'"><strong>Info</strong></td>
                            <td bgcolor="'.$bg1.'">'.$ds['info'].'</td>
                          </tr>';
		}
		else{
		    $show_info = '';
		...
		if($ds['play']) {
		    $show_play = '<tr>
                            <td bgcolor="'.$bg1.'"><strong>Why play?</strong></td>
                            <td bgcolor="'.$bg1.'">'.$ds['play'].'</td>
                          </tr>';
		}
		else{
		    $show_play = '';
