...
elseif(isset($_POST['saveusercomment'])) {
	...
	if(checkCommentsAllow($type,$parentID)){ 
		...
		if($_POST['admin']=="true") safe_query("UPDATE ".PREFIX."cup_tickets SET adminID = '$userID', updated = '".time()."', status = '$admin_reply_status' WHERE ticketID='$parentID'");	
	}
	header("Location: ".$_POST['referer']);
}
	...
	if($gesamt) {
		...
		while($ds=mysqli_fetch_array($ergebnis)) {
			...
			if($ds['userID']) {
				...
				$country=flags($country,$calledfrom = 'admin');
			...
			$content = cleartext($ds['comment'],$bbcode=true, $calledfrom='admin');
			$content = toggle($content, $ds['commentID']);

			if(isfeedbackadmin($userID) or iscommentposter($userID,$ds['commentID'])) {
				$edit = '<a href="../index.php?site=comments&amp;editcomment=true&amp;id='.$ds['commentID'].'&amp;ref=admin/'.urlencode($referer).'" title="'.$_language->module['edit_comment'].'"><img src="../images/icons/edit.gif" border="0" alt="'.$_language->module['edit_comment'].'" /></a>';
			} else $edit='';
	...
	if($comments_allowed) {
	  if($comments_allowed == 1) {
		...
		if(memin($userID,$clanID)) {
		   ...
		   echo 'Only members in '.getclanname($clanID).' can post comments';
