  ...
  if(!ispageadmin($userID) OR mb_substr(basename($_SERVER['REQUEST_URI']),0,15) != "admincenter.php") die($_language->module['access_denied']);
	...
	if($CAPCLASS->check_captcha(0, $_POST['captcha_hash'])) {
		if($upload['name'] != "") {
			...
			while ($table = mysqli_fetch_row($result)) safe_query("DROP TABLE `".$table[0]."`");
	
			move_uploaded_file($upload['tmp_name'], '../tmp/'.$upload['name']);
			$new_query = file('../tmp/'.$upload['name']);
			foreach($new_query as $query) @mysqli_query($query);
...
if($action=="optimize") {
  ...
  $result = mysqli_list_tables($db);
	while ($table = mysqli_fetch_row($result)) safe_query("OPTIMIZE TABLE `".$table[0]."`");
...
elseif($action=="write") {
  ...
  systeminc("func/captcha");
	...
	$result = mysqli_query("SHOW TABLE STATUS FROM ".$db);
	while ($table = mysqli_fetch_row($result)) {
		$i = 0;
		$result2 = mysqli_query("SHOW COLUMNS FROM $table[0]");
		...
		$inhaltq = mysqli_query("SELECT * FROM $table[0]");
