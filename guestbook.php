...
if(isset($_POST['save'])) {
	...
	if($userID) {
		$name = mysqli_real_escape_string(getnickname($userID));
	...
	if($run) {
		if(mb_strlen($_POST['message'])){
			...
			if($gb_info) {
				...
				$message = str_replace('%insertid%', 'id_'.mysqli_insert_id(), mysqli_real_escape_string($_language->module['pmtext_newentry']));
				foreach($touser as $id) {
					sendmessage($id,mysqli_real_escape_string($_language->module['pmsubject_newentry']),$message);
				}
			}
